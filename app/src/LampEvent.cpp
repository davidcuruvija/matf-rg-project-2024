...
void LampEvent::update_lamp(float delta_time) {
    ...
    if (lamp_state == TRANSPARENT) { main_controller->point_light.diffuse = glm::vec3(1.0f); } else if (lamp_state == GREEN) { main_controller->point_light.diffuse = glm::vec3(0.0f, 0.75f, 0.0f); } else if (lamp_state == BLUE) { main_controller->point_light.diffuse = glm::vec3(0.0f, 0.0f, 0.75f); }
}
