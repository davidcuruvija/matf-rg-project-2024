...
void main() {
    ...
    vec3 spotAmbient = spot_light.ambient * texColor * spotAttenuation * intensity;
    vec3 spotDiffuse = spot_light.diffuse * max(dot(norm, spotDir), 0.0) * texColor * spotAttenuation * intensity;
    vec3 spotReflectDir = reflect(-spotDir, norm);
    float spotSpec = pow(max(dot(viewDir, spotReflectDir), 0.0), point_light.shininess);
    vec3 spotSpecular = spot_light.specular * spotSpec * spotAttenuation * intensity;
